#!/usr/bin/env python3
"""
Generate YOCO-MoE-Dense-3B-A11520 model (F16 and I2_S).

Dense variant of YOCO-MoE-30B-A3B-V3: same YOCO architecture but with
standard SwiGLU FFN instead of MoE experts.
...
Parameter count: ~3.4B (MoE-aligned)

Usage:
  python generate_yoco_moe_dense_3b_d_ffn_11520_models.py [--output-dir DIR]
"""
...
logger = logging.getLogger("generate-yoco-dense-3b-d_ffn-11520")
...
def generate_f16_gguf(model_dir: Path, output_path: Path, config: dict):
    ...
    logger.info("Generating YOCO-MoE-Dense-3B-A11520 F16 GGUF")

    n_layers = config["num_hidden_layers"]

    writer = gguf.GGUFWriter(output_path, gguf.MODEL_ARCH_NAMES[gguf.MODEL_ARCH.BITNET])
    add_model_params(writer, config, "YOCO-MoE-Dense-3B-A11520", gguf.GGMLQuantizationType.F16)
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate YOCO-MoE-Dense-3B-A11520 model (F16 and I2_S)"
    )
    parser.add_argument(
        "--output-dir", type=str,
        default="/data3/huangxin/model_list/yoco-moe-dense-3b-d_ffn-11520",
    ...
    config = YOCO_MOE_DENSE_3B_CONFIG
    n_self = config["num_self_layers"]
    n_cross = config["num_cross_layers"]
    n_layers = config["num_hidden_layers"]
    ...
    hf_dir = output_base / "yoco-moe-dense-3b-d_ffn-11520-hf"
    create_hf_model_dir(hf_dir, config)

    f16_dir = output_base / "yoco-moe-dense-3b-d_ffn-11520-bitnet-f16"
    ...
    if not args.skip_i2s:
        i2s_dir = output_base / "yoco-moe-dense-3b-d_ffn-11520-bitnet-i2s"
    ...
    print("\n" + "=" * 60)
    print("YOCO-MoE-Dense-3B-A11520 Models Generated (F16 + I2_S)!")
    ...
    print(f"\nParams (total): {total_params:,} ({total_params/1e9:.2f}B)")
