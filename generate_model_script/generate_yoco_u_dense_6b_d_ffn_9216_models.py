#!/usr/bin/env python3
"""
Generate YOCO-U-Dense-6B model with d_ffn=9216 (F16 and I2_S).
...
Architecture:
  ...
  Dense FFN (SwiGLU): intermediate_size=9216,
...
Storage params: ~3.0B, Effective compute: ~6B (self-decoder runs 3x)

Usage:
  python generate_yoco_u_dense_6b_d_ffn_9216_models.py [--output-dir DIR]
"""
...
logger = logging.getLogger("generate-yoco-u-dense-6b-d_ffn-9216")
...
def generate_f16_gguf(model_dir: Path, output_path: Path, config: dict):
    ...
    logger.info("Generating YOCO-U-Dense-6B (d_ffn=9216) F16 GGUF")

    n_layers = config["num_hidden_layers"]

    writer = gguf.GGUFWriter(output_path, gguf.MODEL_ARCH_NAMES[gguf.MODEL_ARCH.BITNET])
    add_model_params(writer, config, "YOCO-U-Dense-6B-d_ffn-9216", gguf.GGMLQuantizationType.F16)
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate YOCO-U-Dense-6B (d_ffn=9216, T=3) model (F16 and I2_S)"
    )
    parser.add_argument(
        "--output-dir", type=str,
        default="/data3/huangxin/model_list/yoco-u-dense-6b-d_ffn-9216",
    ...
    hf_dir = output_base / "yoco-u-dense-6b-d_ffn-9216-hf"
    create_hf_model_dir(hf_dir, config)

    f16_dir = output_base / "yoco-u-dense-6b-d_ffn-9216-bitnet-f16"
    ...
    if not args.skip_i2s:
        i2s_dir = output_base / "yoco-u-dense-6b-d_ffn-9216-bitnet-i2s"
    ...
    print("\n" + "=" * 60)
    print("YOCO-U-Dense-6B (d_ffn=9216, T=3) Models Generated (F16 + I2_S)!")
