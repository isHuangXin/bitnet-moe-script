#!/usr/bin/env python3
"""
Generate YOCO-MoE-Dense-3B model (F16 and I2_S).
...
Usage:
  python generate_yoco_moe_dense_3b_models.py [--output-dir DIR]
"""
...
def generate_f16_gguf(model_dir: Path, output_path: Path, config: dict):
    ...
    logger.info("Generating YOCO-MoE-Dense-3B F16 GGUF")

    n_layers = config["num_hidden_layers"]

    writer = gguf.GGUFWriter(output_path, gguf.MODEL_ARCH_NAMES[gguf.MODEL_ARCH.BITNET])
    add_model_params(writer, config, "YOCO-MoE-Dense-3B", gguf.GGMLQuantizationType.F16)
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate YOCO-MoE-Dense-3B model (F16 and I2_S)"
    )
    parser.add_argument(
        "--output-dir", type=str,
    ...
    print("\n" + "=" * 60)
    print("YOCO-MoE-Dense-3B Models Generated (F16 + I2_S)!")
