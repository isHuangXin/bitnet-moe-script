#!/bin/bash
# YOCO-U-Dense-6B (d_ffn=9216): F16 vs I2_S CPU Benchmark
# YOCO-U universal loop: 20 stored layers (10 self + 10 cross), self-decoder looped T=3
# ~3.0B stored params, ~6B effective compute, Dense SwiGLU FFN (no MoE)
#
# Usage: bash benchmark_yoco_u_dense_6b_d_ffn_9216_cpu.sh [threads] [numa_node]
# Examples:
#   bash benchmark_yoco_u_dense_6b_d_ffn_9216_cpu.sh 8
#   bash benchmark_yoco_u_dense_6b_d_ffn_9216_cpu.sh 8 0

set -e

MODEL_DIR="/data3/huangxin/model_list/yoco-u-dense-6b-d_ffn-9216"
F16_MODEL="${MODEL_DIR}/yoco-u-dense-6b-d_ffn-9216-bitnet-f16/ggml-model-f16.gguf"
I2S_MODEL="${MODEL_DIR}/yoco-u-dense-6b-d_ffn-9216-bitnet-i2s/ggml-model-i2_s.gguf"
...
echo "========================================================"
echo "  YOCO-U-Dense-6B (d_ffn=9216): F16 vs I2_S Benchmark"
...
echo "  Dense FFN (SwiGLU): intermediate_size=9216"
echo "  yoco_u_iters=3 (self-decoder looped T=3)"
echo "  Stored params: ~3.0B, Effective compute: ~6B"
...
for m in "$F16_MODEL" "$I2S_MODEL"; do
    if [ ! -f "$m" ]; then
        ...
        echo "Run generate_yoco_u_dense_6b_d_ffn_9216_models.py first."
        exit 1
    fi
done
