#!/bin/bash
# YOCO-U-Dense-3B (d_ffn=11520): F16 vs I2_S CPU Benchmark
# YOCO-U universal loop: 20 stored layers (10 self + 10 cross), self-decoder looped T=3
# ~3.4B stored params, Dense SwiGLU FFN (no MoE)
#
# Usage: bash benchmark_yoco_dense_3b_d_ffn_11520_cpu.sh [threads] [numa_node]
# Examples:
#   bash benchmark_yoco_dense_3b_d_ffn_11520_cpu.sh 8
#   bash benchmark_yoco_dense_3b_d_ffn_11520_cpu.sh 8 0

set -e

MODEL_DIR="/data3/huangxin/model_list/yoco-dense-3b-d_ffn-11520"
F16_MODEL="${MODEL_DIR}/yoco-dense-3b-d_ffn-11520-bitnet-f16/ggml-model-f16.gguf"
I2S_MODEL="${MODEL_DIR}/yoco-dense-3b-d_ffn-11520-bitnet-i2s/ggml-model-i2_s.gguf"
...
echo "========================================================"
echo "  YOCO-U-Dense-3B (d_ffn=11520): F16 vs I2_S Benchmark"
...
echo "  Stored params: ~3.4B, Effective compute: ~6B"
...
for m in "$F16_MODEL" "$I2S_MODEL"; do
    if [ ! -f "$m" ]; then
        ...
        echo "Run generate_yoco_dense_3b_d_ffn_11520_models.py first."
        exit 1
    fi
done
